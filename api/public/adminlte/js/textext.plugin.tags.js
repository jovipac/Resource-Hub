...
(function($) {
  ...
  var p = TextExtTags.prototype,
    ...
    DEFAULT_OPTS = {
      ...
      html: {
        ...
        tag:
          '<div class="text-tag"><div class="text-button"><span class="text-label"/><a class="text-remove"/></div></div>'
      }
    };
  /**
  ...
  p.onBackspaceKeyDown = function(e) {
    ...
    if (self.val().length == 0) self.removeTag(lastTag);
  };
  ...
  p.onPreInvalidate = function(e) {
    ...
    if (lastTag.length > 0) pos.left += lastTag.innerWidth();
    else pos = self._originalPadding;
  ...
  p.onClick = function(e) {
    ...
    function tagClickCallback(newValue, refocus) {
      ...
      if (refocus) core.focusInput();
    }

    if (focus) core.focusInput();
  };
  ...
  p.addTags = function(tags) {
    ...
    for (i = 0; i < tags.length; i++) {
      ...
      if (tag && self.isTagAllowed(tag)) container.append(self.renderTag(tag));
    }
  ...
  p.getTagElement = function(tag) {
    ...
    for (i = 0; i < list.length; i++) {
      item = $(list[i]);
      if (self.itemManager().compareItems(item.data(CSS_TAG), tag)) return item;
    }
