...
(function($, undefined) {
  ...
  if (!stringify) throw new Error("JSON.stringify() not found");
  ...
  function getProperty(source, name) {
    ...
    if (typeof (result = source[fullCamelCaseName]) != UNDEFINED)
      ...
      typeof (result = source[nestedName]) != UNDEFINED &&
      name.length > 0
    )
  ...
  function hookupEvents() {
    ...
    for (event in args) bind(event, args[event]);
  }
  ...
  p.filter = function(list, query) {
    ...
    for (i = 0; i < list.length; i++) {
      item = list[i];
      if (this.itemContains(item, query)) result.push(item);
    }
  ...
  p.initPatches = function() {
    ...
    for (name in source) list.push(name);
  ...
  p.initPlugins = function(plugins, source) {
    ...
    if (typeof plugins == "string") plugins = plugins.split(/\s*,\s*|\s+/g);
    ...
    for (i = 0; i < initList.length; i++) initList[i].init(self);
  };
  ...
  p.opts = function(name) {
    ...
    return typeof result == "undefined"
      ? getProperty(this._defaults, name)
      : result;
  };
  ...
  p.getWeightedEventResponse = function(event, args) {
    ...
    for (var weight in data) maxWeight = Math.max(maxWeight, weight);
  ...
  $(["Down", "Up"]).each(function() {
    ...
    p["onKey" + type] = function(e) {
      ...
      if (keyName) {
        ...
        if (type == "Down") self._lastKeyDown = e.keyCode;
      }
  ...
  p.val = function(value) {
    ...
    if (typeof value === UNDEFINED) return input.val();
  ...
  var textext = ($.fn.textext = function(opts) {
    ...
    return this.map(function() {
      ...
      if (opts == null) return self.data("textext");
