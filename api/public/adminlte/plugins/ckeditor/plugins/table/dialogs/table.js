...
(function() {
  function v(a) {
    for (var f = 0, n = 0, m = 0, p, e = a.$.rows.length; m < e; m++) {
      p = a.$.rows[m];
      for (var d = (f = 0), c, b = p.cells.length; d < b; d++)
        (c = p.cells[d]), (f += c.colSpan);
      f > n && (n = f);
    }
    return n;
  }
    ...
    return {
        ...
        if (a)
          a.on("change", function() {
            var a = this.getStyle("width", ""),
              b = e.getContentElement("info", "txtWidth");
            b && b.setValue(a, !0);
      ...
      onShow: function() {
        var e = a.getSelection(),
          ...
          g = this.getContentElement("info", "txtHeight");
        "tableProperties" == f &&
      ...
      onOk: function() {
        var e = a.getSelection(),
          ...
          c = this._.selectedElement || n("table"),
          b = {};
        ...
        if (b.info) {
          ...
          if (!this._.selectedElement)
            for (
              var h = c.append(n("tbody")),
                f = parseInt(b.txtRows, 10) || 0,
                g = parseInt(b.txtCols, 10) || 0,
                k = 0;
              k < f;
              k++
            )
              for (var l = h.append(n("tr")), m = 0; m < g; m++)
                l.append(n("td")).appendBogus();
          f = b.selHeaders;
          if (!c.$.tHead && ("row" == f || "both" == f)) {
            l = new CKEDITOR.dom.element(c.$.createTHead());
            h = c.getElementsByTag("tbody").getItem(0);
            h = h.getElementsByTag("tr").getItem(0);
            for (k = 0; k < h.getChildCount(); k++)
              (g = h.getChild(k)),
          ...
          if (null !== c.$.tHead && "row" != f && "both" != f) {
            l = new CKEDITOR.dom.element(c.$.tHead);
            h = c.getElementsByTag("tbody").getItem(0);
            for (m = h.getFirst(); 0 < l.getChildCount(); ) {
              h = l.getFirst();
              for (k = 0; k < h.getChildCount(); k++)
                (g = h.getChild(k)),
                  g.type == CKEDITOR.NODE_ELEMENT &&
          ...
          if (this.hasColumnHeaders && "col" != f && "both" != f)
            for (k = 0; k < c.$.rows.length; k++)
              (l = new CKEDITOR.dom.element(c.$.rows[k])),
                "tbody" == l.getParent().getName() &&
                  ((g = new CKEDITOR.dom.element(l.$.cells[0])),
                  g.renameNode("td"),
